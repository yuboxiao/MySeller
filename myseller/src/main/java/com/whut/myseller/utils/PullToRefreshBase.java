...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Styleables from XML
		int[] pullToRefresh = { 0x7f01006a, 0x7f01006b, 0x7f01006c,
				0x7f01006d };
		final TypedArray a = context.obtainStyledAttributes(attrs,
				pullToRefresh);
		int PullToRefresh_mode = 3;
		mode = a.getInteger(PullToRefresh_mode,
		...
		// Styleables from XML
		int PullToRefresh_headerTextColor = 2;
		if (a.hasValue(PullToRefresh_headerTextColor)) {
			final int color = a.getColor(PullToRefresh_headerTextColor, Color.BLACK);
			if (null != headerLayout) {
		...
		int PullToRefresh_headerBackground = 1;
		int PullToRefresh_adapterViewBackground = 0;
		if (a.hasValue(PullToRefresh_headerBackground)) {
			this.setBackgroundResource(a.getResourceId(PullToRefresh_headerBackground, Color.WHITE));
		}
		if (a.hasValue(PullToRefresh_adapterViewBackground)) {
			refreshableView.setBackgroundResource(a.getResourceId(PullToRefresh_adapterViewBackground,
					Color.WHITE));
		}
		a.recycle();
